  ...
  if (fExec && opcodenum >= 0 && opcodenum <= Opcode.OP_PUSHDATA4) {
    ...
    switch (opcodenum) {
        ...
        switch (buf[0]) {
          ...
          case 5: {
            ...
            for (const input of this.tx.inputs) {
                inputsCombinedBuffers.push(Buffer.from(input.prevTxId, 'hex'))
          ...
          case 6:{
            var resultArray = []
            resultArray.push(this.tx.inputs[this.nin].prevTxId.toString('hex')) 
            var outputIndexBuffer = Buffer.alloc(4)
      ...
      case Opcode.OP_PARTIAL_HASH:
        ...
        bufCompleteSize = stacktop(-1)
        bufPartialHash = stacktop(-2)
        bufData = stacktop(-3)
        this.stack.pop()
        this.stack.pop()
        this.stack.pop()
        var bufHash
        if (bufPartialHash.length == 0 && bufData.length == bufCompleteSize) {
          bufHash = Hash.sha256(bufData)
        } else if (bufPartialHash.length != 32) {
          this.errstr = 'SCRIPT_ERR_INVALID_STACK_OPERATION'
          return false
        } else {
          bufHash = partial_sha256.compute_partial_hash(bufData, bufPartialHash, bufCompleteSize)
        }
