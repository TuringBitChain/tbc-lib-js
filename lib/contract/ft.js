...
class FT {
    ...
    fetchTXraw(txid) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const response = yield fetch(url_mainnet);


                if (!response.ok) {
                    const errorMessage = `Failed to fetch TXraw. Status: ${response.status} ${response.statusText}`;
                    throw new Error(errorMessage);
                }


                const rawtx = yield response.json();
    ...
    fetchFtTXO(contractTxid, addressOrHash, amount) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const response_unspent = yield fetch(url_mainnet);


                if (!response_unspent.ok) {
                    throw new Error(`Failed to fetch FT UTXO: ${response_unspent.statusText}`);
                }

                const responseData = yield response_unspent.json();
                const utxoList = responseData.ftUtxoList;

                if (!utxoList || utxoList.length === 0) {
                    throw new Error('No UTXOs found for this contract.');
                }

                let data = utxoList[0];
                for (let i = 0; i < utxoList.length; i++) {
                    if (utxoList[i].ContractBalance > amount) {
                        data = utxoList[i];
                        break;
                    }
                }
                ...
                console.error('Error fetching FT UTXO:', error);
    ...
    static getFTbalance(contractTxid, addressOrHash) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (!response_unspent.ok) {
                    throw new Error(`Failed to get FT balance: ${response_unspent.statusText}`);
                }

                const responseData = yield response_unspent.json();
                let ftBalance = 0;
                for (let i = 0; i < responseData.length; i++) {
                    ftBalance += responseData[i].ContractBalance;
                }
                return ftBalance;
            } catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                for (let i = 0; i < data.length; i++) {
                    if (data[i].value > 5000) {
                        selectedData = data[i];
                        break;
                    }
                }

                if (selectedData.value < 5000) {
                    throw new Error('UTXO value is less than 5000');
                }

                const utxo = {
                    txId: selectedData.tx_hash,
                    outputIndex: selectedData.tx_pos,
                    script: scriptPubKey,
                    satoshis: selectedData.value
                };
                return utxo;
            } catch (error) {
